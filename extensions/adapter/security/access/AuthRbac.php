...
class AuthRbac {
     ...
     * @SuppressWarnings("php:S105")
     */
    protected $_autoConfig = ['roles'];
     ...
     */
    public function check($requester, $params, array $options = []) {
        ...
        return !$accessible ? compact('message', 'redirect') : [];
    }
     ...
     * @return boolean
     */
    protected static function _isAccessible(array &$role, $params, $options): bool
    {
        if ($role['allow'] === false) {
            return false;
        }
    ...
    public static function parseMatch($match, $params) {
        ...
        foreach ((array) $match as $key => $param) {
            if (is_string($param) && preg_match('/^([A-Za-z0-9_\*\\\]+)::([A-Za-z0-9_\*]+)$/', $param, $regexMatches)) {
                $matchParams += array(
     ...
     * finds. Returns true if all the closures it runs evaluate to true. $match
